...
with open('dollar.json') as jsondata:
	d = json.load(jsondata)
	for i in d:
		regex = re.search(r'(\d\d)(\d\d)(\d\d)', i["orig"])
		if regex:
			yearFormat = regex.group(1)
			monthFormat = regex.group(2) # can delete
			dayFormat = regex.group(3) # can delete

			#newYear = yearFormat[0:2] + "-" + yearFormat[2:4] + "-" + yearFormat[4:6]
...
for ep in extract:
	if ep[1] in txtExtractYears:
		...
		c.execute("INSERT INTO radio VALUES(?, ?, ?, ?, ?)", (title, "Yours Truly, Johnny Dollar", "Detective", year, mp3))
